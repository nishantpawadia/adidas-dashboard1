...
try:
    ...
    st.error("Adidas.xlsx file not found in current directory. Please place the file in the working folder.")
    st.stop()

if "InvoiceDate" not in df.columns or "Retailer" not in df.columns or "TotalSales" not in df.columns:
    st.error("Adidas.xlsx must contain columns InvoiceDate, Retailer, TotalSales, State, UnitsSold, Region, City.")
    st.stop()
...
try:
    ...
    st.error("Failed to convert InvoiceDate to datetime.")
    st.stop()

if df["InvoiceDate"].isna().all():
    st.error("InvoiceDate has no valid dates after parsing.")
    st.stop()
...
try:
    ...
    image = None
    st.warning("adidas-logo.jpg not found; logo will not be displayed.")
